...
import { BubbleItem } from "@biseo/web/components/molecules/BubbleItem";
  ...
  return (
    <div css={[column, gap(4), padding.horizontal(20), padding.vertical(10)]}>
      <div css={[row, justify.between, align.center]}>
        ...
        {isAdmin && isMy && (
          <BubbleItem
            label={isNotice ? "공지 해제" : "공지로 설정"}
            position="top"
          >
            <Megaphone
              size={20}
              color={isNotice ? colors.blue600 : colors.gray300}
              css={css`
                cursor: pointer;
              `}
              onClick={() => updateMessageType(message.id, nextType)}
            />
          </BubbleItem>
        )}
      </div>
